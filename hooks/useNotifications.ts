...
export function useNotifications({ role, tableId, userId }: UseNotificationsOptions) {
  ...
  const handleMessage = useCallback(
    (msg: WsMessage) => {
      ...
      switch (data.status) {
        case "pending":
          title = "Nuevo pedido";
          message = `Mesa ${data.table_id ? "#" : "Para llevar"} - ${data.customer_name}`;
          break;
        case "in_preparation":
  ...
  return { notifications, unreadCount, connected, markRead, markAllRead, clearAll };
}
