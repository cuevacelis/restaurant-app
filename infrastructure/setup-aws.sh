...
echo "       pnpm db:seed"
echo ""
echo "  3) Activar la extensión aws_lambda en RDS (solo la primera vez):"
echo -e "  ${YELLOW}   psql -h \$DB_HOST -U \$DB_USER -d \$DB_NAME \\"
echo -e "       -c 'CREATE EXTENSION IF NOT EXISTS aws_lambda CASCADE;'${NC}"
echo ""
echo "  4) Instalar triggers de tiempo real:"
echo -e "  ${YELLOW}   psql -h \$DB_HOST -U \$DB_USER -d \$DB_NAME \\"
echo -e "       -v lambda_arn='${TRIGGER_ARN}' \\"
echo -e "       -f infrastructure/sql/triggers.sql${NC}"
echo ""
echo "  5) Iniciar la app:"
echo "       pnpm dev"
echo "============================================================"
